...
/**
 * The Class SquareGenerator.
 */
public class SquareGenerator extends AbstractNumericGenerator {

    /** The Constant LOGGER. */
    private static final Logger LOGGER = LoggerFactory.getLogger(SquareGenerator.class);
    ...
    public void generate(final long start, final long end, final long step, final ValuesWriter writer) throws IOException {
        ...
        if (noiseAmplitude > 0) {
            ...
            f = t -> Double.valueOf(Math.signum(Math.sin(t / 1000.0 * Constants.PI2 * frequency)) * amplitude + offset + random.nextGaussian() * noiseAmplitude);
        } else {
            f = t -> Double.valueOf(Math.signum(Math.sin(t / 1000.0 * Constants.PI2 * frequency)) * amplitude + offset);
        }
